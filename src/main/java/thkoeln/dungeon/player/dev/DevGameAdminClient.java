...
public class DevGameAdminClient {
    ...
    public void endAllGames() {
        ...
        for ( GameDto gameDto : gameDtos ) {
            if ( gameDto.getGameStatus() == GameStatus.CREATED ) {
                log.debug( DEV_PREFIX + "Found 'created' game, which needs to be started first: " + gameDto.getGameId() );
                startGame( gameDto.getGameId() );
                log.debug( DEV_PREFIX + "... wait a bit before actually ending the game ..." );
                Thread.sleep( 15000 );
            }
    ...
    private void endGame( UUID gameId ) {
        log.info( DEV_PREFIX + "endGame() for this game: " + gameId );
    ...
    private void startGame( UUID gameId ) {
        log.info( DEV_PREFIX + "gameStarted()" );
