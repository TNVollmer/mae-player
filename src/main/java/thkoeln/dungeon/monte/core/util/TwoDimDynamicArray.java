...
public class TwoDimDynamicArray<T> {
    ...
    public TwoDimDynamicArray( int maxX, int maxY ) {
        if ( maxX <= 0 || maxY <= 0 )
            throw new UtilException( "maxX / maxY must be > 0: " + maxX + ", " + maxY );
    ...
    public T at( Coordinate coordinate ) {
        if ( coordinate == null ) throw new UtilException( "coordinate must not be null" );
        if ( coordinate.isLargerThan( getMaxCoordinate() ) )
            throw new UtilException( "coordinate out of bounds: " + coordinate );
    ...
    public Coordinate putAndEnhance(Coordinate coordinate, CompassDirection compassDirection, T value ) {
        ...
        if ( coordinate.isLargerThan( getMaxCoordinate() ) )
            throw new UtilException( "coordinate out of bounds: " + coordinate );
        if ( compassDirection == null ) throw new UtilException( "compassDirection must not be null" );
    ...
    public void addRowAt( int y ) {
        if ( y < 0 ) throw new UtilException( "can't add row at index < 0: " + y );
        if ( y > sizeY() ) throw new UtilException( "can't add row at index > sizeY: " + y );
        if ( y < sizeY() ) array.add( y, createNullRow( sizeX() ) );
        else array.add( createNullRow( sizeX() ) );
    }

    public void addColumnAt( int x ) {
        if( x < 0 ) throw new UtilException( "can't add column at index < 0: " + x );
        if( x > sizeX() ) throw new UtilException( "can't add column at index > sizeX: " + x );
    ...
    public Coordinate find( T element ) {
        if ( element == null ) throw new UtilException( "element == null" );
        int y = 0;
